...
public class StreamLocationDao extends JooqDao<StreamLocation>
{
    ...
    public Set<StreamLocation> getStreamLocations(String streamId, String unitSystem, String officeId) throws SQLException
    {
        ...
        String pStationUnitIn = UnitSystem.EN.value().equalsIgnoreCase(unitSystem) ? Unit.MILE.getValue() : Unit.KILOMETER.getValue();
        String pStageUnitIn = UnitSystem.EN.value().equalsIgnoreCase(unitSystem) ? Unit.FEET.getValue() : Unit.METER.getValue();
        String pAreaUnitIn = UnitSystem.EN.value().equalsIgnoreCase(unitSystem) ? Unit.SQUARE_MILES.getValue() : Unit.SQUARE_KILOMETERS.getValue();
        CwmsDbStreamJooq streamJooq = new CwmsDbStreamJooq();
        AtomicReference<ResultSet> resultSetRef = new AtomicReference<>();
        dsl.connection(c -> resultSetRef.set(streamJooq.catStreamLocations(c, pStreamIdMaskIn, pLocationIdMaskIn, pStationUnitIn, pStageUnitIn, pAreaUnitIn, officeId)));
    ...
    private Set<StreamLocation> buildStreamLocations(ResultSet rs) throws SQLException
    {
        ...
        while(rs.next())
        {
            ...
            StreamLocation loc = new StreamLocation.Builder(locationId, streamId, station, bank, officeId)
