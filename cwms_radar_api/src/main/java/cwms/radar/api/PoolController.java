...
public class PoolController implements CrudHandler {
    ...
    @OpenApi(queryParams = {
            ...
            @OpenApiParam(name = "bottom-mask", description = "Bottom level mask. Default "
                    + "value:*\""),
            @OpenApiParam(name = "top-mask", description = "Top level mask. Default value:*\""),
            @OpenApiParam(name = "include-explicit", description = "Specifies if the results "
                    + "should include explicit Pools. Default value:false\""),
            @OpenApiParam(name = "include-implicit", description = "Specifies if the results "
    ...
    public void getAll(Context ctx) {
        ...
        try (final Timer.Context timeContext = getAllRequestsTime.time();
            ...
            String projectIdMask =
                    ctx.queryParamAsClass("id-mask", String.class).getOrDefault(ANY_MASK);
            String nameMask =
                    ctx.queryParamAsClass("name-mask", String.class).getOrDefault(ANY_MASK);
            String bottomMask =
                    ctx.queryParamAsClass("bottom-mask", String.class).getOrDefault(ANY_MASK);
            String topMask = ctx.queryParamAsClass("top-mask", String.class).getOrDefault(ANY_MASK);

            String isExp = ctx.queryParamAsClass("include-explicit", String.class).getOrDefault(
                    "false");
            boolean isExplicit = Boolean.parseBoolean(isExp);
            String isImp = ctx.queryParamAsClass("include-implicit", String.class)
                    .getOrDefault("true");
    ...
    @OpenApi(
            pathParams = {
                    @OpenApiParam(name = "pool-id", required = true, description = "Specifies the"
            ...
            queryParams = {
                    @OpenApiParam(name = "office", required = true, description = "Specifies the "
                            ...
                            + "response."),
                    @OpenApiParam(name = "project-id", required = true, description = "Specifies "
                            + "the project-id of the Pool whose data is to be included in the "
                            + "response."),
                    @OpenApiParam(name = "bottom-mask", description = "Bottom level mask. Default"
                            + " value:*\""),
                    @OpenApiParam(name = "top-mask", description = "Top level mask. Default "
                            + "value:*\""),
                    @OpenApiParam(name = "include-explicit", description = "Specifies if the "
                            + "results should include explicit Pools. Default value:false\""),
                    @OpenApiParam(name = "include-implicit", description = "Specifies if the "
    ...
    public void getOne(Context ctx, String poolId) {
        ...
        try (final Timer.Context timeContext = getOneRequestTime.time();
            ...
            String isExp = ctx.queryParamAsClass("include-explicit", String.class).getOrDefault(
                    "true");
            boolean isExplicit = Boolean.parseBoolean(isExp);
            String isImp = ctx.queryParamAsClass("include-implicit", String.class).getOrDefault(
                    "true");
            boolean isImplicit = Boolean.parseBoolean(isImp);
