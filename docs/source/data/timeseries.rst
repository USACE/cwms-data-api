...
we are measuring or have calculated. For example "Stage" is the height of water in a river, generally from its lowest point.
...
concepts that may seem odd, in some cases they are but useful for description, in others they are actively useful in
different concepts; explaining which is which or why one would be used is beyond the scope of this document.

Every "measurement", or "calculated value", often referred to as a "sample", which we will use below, has a time of measurement, a value, and what
we are measuring.
...
increases a counter based on calibrated volume, with the data logger then reading and resetting to 0 that value on some
...
Another form of average is a "moving average" or "running average" depending on what text you are reading.
...
.. csv-table:: Types
    ...
    Inst, Instantaneous, Samples measured \"now\". For example if you take a measuring tape and measure the length of a piece of wood.
    ...
    Const, Constant, "Sample value doesn't change. Usually with a duration of UntilChanged."
...
.. csv-table:: Intervals (not exhaustive list)
    ...
    defined in the local time zone. Accounts for the normal 24 hour day, as well as the 23 and 25 hour days around
...
.. csv-table:: Durations (not exhaustive list)
    ...
    Should a given value be stored that is not calculated in this way it will have a duration ending in BOP."
    1Hour, 1Hour, "Sample is a composite of input data over a 1 hour window regardless of the number of input samples
    in that window."
    Variable, Variable, "Data is not instantaneous, but also does not have a fixed window."
    UntilChanged, Until Changed, "Value is a constant between each sample. E.g. value should not be interpolated between
    measurements. Just used as-is until another is provided."

BOP vs EOP
~~~~~~~~~~

If one looks at various reports from USACE you will see the time value "2400", which many may surmise does not actually
exist. This is a convention to indicate "At the end of the given day."

An End of Period measurement of a 1 Day average of hourly data, from say `2026-06-30T00:00:00` to `2026-07-01T00:00:00`
would store the resulting single output value at `2026-07-01T00:00:00` (as no database except HEC-DSS directly allows times at `2400`).
In reports using the `2400` time value this would be for `2026/06/30 @ 2400`. This possible ambiguity is why durations
...
*all* stored data in CWMS is stored at the non-ambiguous UTC time. So if a district wants to maintain a `7AM Local Time`
...
be 7 hours. All samples stored to that time series are checked to confirm the input data matches that offset and rejected
...
A time series is of regular interval if, **and only if** the times between samples are a constant amount of seconds.
...
A time series where there is no expectation of when a measurement will be provided is irregular. Any sample time
(except duplicates) is valid, and there is no automated way to distinguish if a sample is missing or never existed.
In this case the sample times are not predictable.
(In CWMS one can store an artificial "missing value", e.g. a sample with a time but no value, when it is known something
should be present)
...
The sample times *may* or *may not* be predictable within a given application context; however, the CWMS database (and
...
As all data is stored in UTC in the CWMS Database, when Daily, and down to 2 Hour interval data, the offset will change
...
Several districts have been using the PseudoRegular 1Day (\~1Day) interval to do this, and manually enforcing the
required exclusion or storing the "missing values" so various reports would line up.

There is an additional confusion. The first attempt to allow for the Local Regular nature of a time series to be
determined was by combining the visual `~<Interval>` with meta data stored on the time series. This made it non-obvious
...
`X-CWMS-LRTS-Formatting`, and if set to a value of `true`, will show (as well as expect on input) Local Regular Time Series
