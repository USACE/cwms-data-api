...
.. list-table::
   ...
   * - Optional columns
     - The following columns are optional and off by default: time-series-id, office-id, version-date, data-entry-date, quality. Everything except date-time and value (with units in the header) is optional.
   * - Meta-data fields
   ...
   * - Units location
     - Units are always expressed in the value column header via parentheses (e.g., ``value (m^3/s)``) and are not repeated elsewhere. We do not include units as a separate column nor in metadata comments. We avoid the anti-pattern of dual representation of the same semantic field; units must exist in exactly one canonical location in all modes.
