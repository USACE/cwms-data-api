====================================
JMS Pub/Sub Message Formats - Levels
====================================


Summary
=======

CWMS needs a JMS Pulish/Subscribe message structure to notify clients of location level-related events.

Opinions
========

Opinion 1
---------

Summary: Use the ``jakarta.jms.MapMessage`` structure described below for level-related-related events.

All messages will be published to the ``XXX_REALTIME_OPS`` topic, where ``XXX`` represents the appropriate office identifier.

Author: Mike Perryman

**Specified Levels**

+-----------------------+--------------------------------------------------------------------------------+
| Message Type          | Structure                                                                      |
+=======================+================================================================================+
| SpecifiedLevelCreated | +------------+----------------------+----------------------------------------+ |
|                       | | Value Type | Value Name           | Value                                  | |
|                       | +============+======================+========================================+ |
|                       | | String     | "type"               | "specified_level_created"              | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "office_id"          | The office identifier                  | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "specified_level_id" | The specified level identifier         | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "description"        | The description of the specified level | |
|                       | +------------+----------------------+----------------------------------------+ |
+-----------------------+--------------------------------------------------------------------------------+
| SpecifiedLevelUpdated | +------------+----------------------+----------------------------------------+ |
|                       | | Value Type | Value Name           | Value                                  | |
|                       | +============+======================+========================================+ |
|                       | | String     | "type"               | "specified_level_updated"              | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "office_id"          | The office identifier                  | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "specified_level_id" | The specified level identifier         | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "description"        | The description of the specified level | |
|                       | +------------+----------------------+----------------------------------------+ |
+-----------------------+--------------------------------------------------------------------------------+
| SpecifiedLevelDeleted | +------------+----------------------+----------------------------------------+ |
|                       | | Value Type | Value Name           | Value                                  | |
|                       | +============+======================+========================================+ |
|                       | | String     | "type"               | "specified_level_deleted"              | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "office_id"          | The office identifier                  | |
|                       | +------------+----------------------+----------------------------------------+ |
|                       | | String     | "specified_level_id" | The specified level identifier         | |
|                       | +------------+----------------------+----------------------------------------+ |
+-----------------------+--------------------------------------------------------------------------------+

**Location Levels**

+----------------------+---------------------------------------------------------------------------------------------------------------+
| Message Type         | Structure                                                                                                     |
+======================+===============================================================================================================+
| LocationLevelCreated | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | Value Type | Value Name            | Value                                                                | |
|                      | +============+=======================+======================================================================+ |
|                      | | String     | "type"                | "location_level_created"                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "office_id"           | The office identifier                                                | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "location_level_id"   | The location level identifier                                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_id"        | The attribute identifier                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | double     | "attribute_value"     | The value of the attribute                                           | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_unit"      | The unit of the attribute                                            | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "effective_time"      | The effective date/time of the location level in epoch milliseconds  | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | boolean    | "virtual"             | Whether the level is a virtual location level                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | double     | "constant_value"      | The value of the level if it is constant                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "constant_value_unit" | The unit of the constant value                                       | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "interval_origin"     | The date/time of the beginning of any interval in epoch milliseconds | |
|                      | |            |                       | if the level is regularly varying (seasonal)                         | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | int        | "interval_months"     | The calendar-based recurrence interval if the level is regularly     | |
|                      | |            |                       | varying (seasonal)                                                   | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | int        | "interval_minutes"    | The time-based recurrence interval if the level is regularly varying | |
|                      | | int        | "interval_minutes"    | (seasonal)                                                           | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "time_series_id"      | The identifier of the backing time series if the level is            | |
|                      | |            |                       | irregularly varying                                                  | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "expiration_time"     | The date/time the level expires in epoch milliseconds                | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "level_comment"       | The comment on the location level                                    | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_comment"   | The comment on the level attribute                                   | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
+----------------------+---------------------------------------------------------------------------------------------------------------+
| LocationLevelUpdated | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | Value Type | Value Name            | Value                                                                | |
|                      | +============+=======================+======================================================================+ |
|                      | | String     | "type"                | "location_level_updated"                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "office_id"           | The office identifier                                                | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "location_level_id"   | The location level identifier                                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_id"        | The attribute identifier                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | double     | "attribute_value"     | The value of the attribute                                           | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_unit"      | The unit of the attribute                                            | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "effective_time"      | The effective date/time of the location level in epoch milliseconds  | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | boolean    | "virtual"             | Whether the level is a virtual location level                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | double     | "constant_value"      | The value of the level if it is constant                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "constant_value_unit" | The unit of the constant value                                       | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "interval_origin"     | The date/time of the beginning of any interval in epoch milliseconds | |
|                      | |            |                       | if the level is regularly varying (seasonal)                         | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | int        | "interval_months"     | The calendar-based recurrence interval if the level is regularly     | |
|                      | |            |                       | varying (seasonal)                                                   | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | int        | "interval_minutes"    | The time-based recurrence interval if the level is regularly varying | |
|                      | | int        | "interval_minutes"    | (seasonal)                                                           | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "time_series_id"      | The identifier of the backing time series if the level is            | |
|                      | |            |                       | irregularly varying                                                  | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "expiration_time"     | The date/time the level expires in epoch milliseconds                | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "level_comment"       | The comment on the location level                                    | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_comment"   | The comment on the level attribute                                   | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
+----------------------+---------------------------------------------------------------------------------------------------------------+
| LocationLevelDeleted | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | Value Type | Value Name            | Value                                                                | |
|                      | +============+=======================+======================================================================+ |
|                      | | String     | "type"                | "location_level_deleted"                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "office_id"           | The office identifier                                                | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "location_level_id"   | The location level identifier                                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_id"        | The attribute identifier                                             | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | double     | "attribute_value"     | The value of the attribute                                           | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | String     | "attribute_unit"      | The unit of the attribute                                            | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | long       | "effective_time"      | The effective date/time of the location level in epoch milliseconds  | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
|                      | | boolean    | "virtual"             | Whether the level is a virtual location level                        | |
|                      | +------------+-----------------------+----------------------------------------------------------------------+ |
+----------------------+---------------------------------------------------------------------------------------------------------------+


Decision Status
===============

Status: request for comments

References
==========
