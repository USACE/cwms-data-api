...
import static cwms.cda.api.Controllers.ALL_DOWNSTREAM;
...
final class DownstreamLocationsGetControllerIT extends DataApiTestIT {
    ...
    @Test
    void test_getDownstreamLocations() throws IOException {
        ...
        //retrieve downstream locations
        given()
                .log().ifValidationFails(LogDetail.ALL, true)
                .accept(Formats.JSON)
                .queryParam(ALL_DOWNSTREAM, true)
        ...
        .when()
                .redirects().follow(true)
                .redirects().max(3)
        .get("/stream-locations/" + streamLocation.getId().getOfficeId() + "/" + streamLocationId + "/downstream-locations")
                .then()
                .log().ifValidationFails(LogDetail.ALL, true)
        .assertThat()
                .statusCode(is(HttpServletResponse.SC_OK))
                .body("[0].stream-location-node.id.office-id", equalTo(streamLocation2.getId().getOfficeId()))
                .body("[0].stream-location-node.id.name", equalTo(streamLocation2.getId().getName()))
                .body("[0].stream-location-node.stream-node.stream-id.office-id", equalTo(streamLocation2.getStreamId().getOfficeId()))
                .body("[0].stream-location-node.stream-node.stream-id.name", equalTo(streamLocation2.getStreamId().getName()))
                .body("[0].stream-location-node.stream-node.bank", equalTo(streamLocation2.getStreamLocationNode().getStreamNode().getBank().getCode()))
                .body("[0].stream-location-node.stream-node.station", equalTo(streamLocation2.getStreamLocationNode().getStreamNode().getStation().floatValue()))
                .body("[0].stream-location-node.stream-node.station-units", equalTo(streamLocation2.getStreamLocationNode().getStreamNode().getStationUnits()))
                .body("[0].published-station", equalTo(streamLocation2.getPublishedStation().floatValue()))
                .body("[0].navigation-station", equalTo(streamLocation2.getNavigationStation().floatValue()))
                .body("[0].lowest-measurable-stage", equalTo(streamLocation2.getLowestMeasurableStage().floatValue()))
                .body("[0].total-drainage-area", equalTo(streamLocation2.getTotalDrainageArea().floatValue()))
                .body("[0].ungaged-drainage-area", equalTo(streamLocation2.getUngagedDrainageArea().floatValue()))
                .body("[0].area-units", equalTo(streamLocation2.getAreaUnits()))
                .body("[0].stage-units", equalTo(streamLocation2.getStageUnits()));
